...
def generateTestPoses(root_fp: str, scene: str, numberOfFrames: int, ds: int, render_height_factor: float):
    ...
    Args:
        ...
        render_height_factor (float): Multiplier for adjusting the height of the camera from the mean height 
    ...
    with open(os.path.join(os.path.join(root_fp, scene), 'transforms_train.json'), 'r') as fp:
        meta = json.load(fp)

    frame = meta["frames"][0]
    c2w = np.array([frame["transform_matrix"] for frame in meta["frames"]])
    height = np.ceil(np.linalg.norm(c2w[:, :-1, -1], axis=1).mean() * render_height_factor)
    print(f'Spherical Pose Height Used: {height}')
    ...
    render_aabb = list(np.concatenate(meta['target_aabb']).flat) if 'target_aabb' in meta else aabb
    camtoworlds = sphericalPoses(numberOfFrames, radius=height)
    enu_center = meta["enu_center"]

    return camtoworlds, K, int(w), int(h), aabb, num_train_img, enu_center, render_aabb


def generateTestPosesFromImages(root_fp: str, scene: str, image_id: List[int], ds: int):
    """This function generates test poses for rendering from a trained model

    Args:
        root_fp (str): Root folder of all scenes
        scene (str): The name of the scene
        image_id (list: int): Which image to render
        ds (int): Downscale factor from original image size
    ...
    for i in image_id:
        frame = meta['frames'][i]
        c2w.append(frame["transform_matrix"])
        f = frame['fl_x'] / ds
        x = frame['cx'] / ds
        y = frame['cy'] / ds
        focal.append(f) 
        cx.append(x)
        cy.append(y)
        K.append([[f, 0, x], [0, f, y], [0, 0, 1]])
    
    c2w, focal, K = np.array(c2w), np.array(focal), np.array(K)
    cx, cy = np.array(cx), np.array(cy)
    w, h = int(frame['w']/ds), int(frame['h']/ds)
    num_train_img = len(meta["frames"])
    
    #intrinsics, bounding box, c2w, and ROI enu_center (in lat/lon)
    aabb = list(np.concatenate(meta['aabb']).flat)
    enu_center = meta["enu_center"]

    return c2w, K, w, h, aabb, num_train_img, enu_center
