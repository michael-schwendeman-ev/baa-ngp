...
def evaluate_test_time_photometric_optim(
    ...
    se3_refine_test = torch.nn.Parameter(torch.zeros(3, device=device))
    ...
    radiance_field.eval()
    estimator.eval()
    pose_refine_test = so3_to_SE3(se3_refine_test)
    # for it in iterator:
    #     pose_refine_test = so3_to_SE3(se3_refine_test)
